...
module Rage
  class Aggregator
    ...
    def update_hour(hour)
      trades = Rage.redis.smembers(hour)
      count = trades.count
      ...
      Rage.redis.hmset('agg:' + hour, 'sma', sma, 'trades', count, 'high', high, 'low', low, 'volume', volume, 'open', open_price[2].to_f, 'close', close_price[2].to_f)
    end
    ...
    def save_trades(trades)
      ...
      trades.each do |trade|
        time = Time.now
        hour = key(Time.now)
        Rage.redis.zadd("mtgox:trades:#{today(time)}", time.to_i, "mtgox:trade:#{trade.id}")
